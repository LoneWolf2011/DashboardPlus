...
* - getLocationRmsComp()
* - getLocationRmsPath()
* - getLocationRmsVoeding()
* - getLocationRmsPoll()
* - getLocationSCSValues()
* - getLocationSCSComp()
* - getLocationSCSEvents()
* Classes list:
* - Location
*/
class Location
{
    ...
    public function getLocation($post_val)
    {
        ...
        $result = $conn->query("SELECT
				...
				AND scs_account_address.SCS_Account_Nmbr LIKE ?s", '%'.$account_nr.'%');
    ...
    public function getLocationRmsComp($post_val)
    {
        ...
        $path_status = $conn_scs->query("SELECT
				...
				AND scs_account_status.SCS_Account_Nmbr LIKE ?s", '%'.$account_nr.'%');
        ...
        $rms_res = $conn->query("SELECT 
				...
				FROM rms_status_db 
				WHERE I_MAC_ETH0 = ?s 
				ORDER BY Diag_scan_id DESC Limit 20", $row_path['SCS_Account_CallerID_1']);
    ...
    public function getLocationRmsPath($post_val)
    {
        ...
        {
            $ip_diag = $conn->query("SELECT Diag_date, S_PAC_no_" . $i . "_STATE FROM rms_status_db WHERE I_MAC_ETH0 LIKE ?s ORDER BY Diag_scan_id DESC limit 25;", '%'.$row_path['SCS_Account_CallerID_1'].'%');
            $ip_arr = array();
            $ip_arr_date = array();
            $ip_arr_count = array();
        ...
        $ip_total = $conn->query("SELECT Diag_scan_id,
										...
										D_ACCOUNT_CODE
										FROM rms_status_db 
										WHERE I_MAC_ETH0 LIKE ?s ORDER BY Diag_scan_id DESC limit 25;", '%'.$row_path['SCS_Account_CallerID_1'].'%');
        ...
        while ($res = $conn->fetch($ip_total))
        {
            ...
            $primair = array(
                @$res['S_PAC_no_1_STATE'],
                @$res['S_PAC_no_3_STATE']
            );
            $secundair = array(
                @$res['S_PAC_no_2_STATE'],
                @$res['S_PAC_no_4_STATE']
            );
    ...
    public function getLocationRmsPoll($post_val)
    {
        ...
        for ($i = 1;$i < $path_arr_count;$i++)
        {
            ...
            $poll_diag = $conn->query("SELECT Diag_date, S_PAC_no_" . $i . "_POLL_DELAY_COUNT  FROM rms_status_db WHERE I_MAC_ETH0 LIKE ?s ORDER BY Diag_scan_id DESC limit 25;", '%' . $row_path['SCS_Account_CallerID_1'] . '%');
    ...
    public function getLocationSCSEvents($post_val)
    {
        ...
        $result = $conn->query("SELECT 	COUNT(event_zone) AS count_events,
									...
									WHERE scs_account_nmbr LIKE ?s
									AND Event_Text NOT LIKE 'Herstel:%'
									GROUP BY event_zone", '%' . $account_nr . '%');
        ...
        while ($row = $conn->fetch($result))
        {
            ...
            array_push($response_array['events'], $test = array(
        ...
        $response_array['count'] = $conn->getOne("SELECT COUNT(event_zone) FROM events" . date('Y') . "_" . $week_nr . ".event_received WHERE scs_account_nmbr LIKE ?s ", '%' . $account_nr . '%');
