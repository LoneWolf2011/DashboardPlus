...
class User
{
    ...
    public function newUser($post_val)
    {
        ...
        if ($check_email)
        {
            ...
            $response_array['body'] = LANG['user']['acc_update']['msg']['err_email_in_use']['text'] . ': <b>' . $post_val['user_email'] . '</b>';
        ...
        if ($conn->query("INSERT INTO app_users SET ?u", $query_data))
        {
            ...
            if ($mail->Send())
            {
                $send = LANG['users']['new']['msg']['login_email']['text'] . ': <b>' . $post_val['user_email'] . '</b>';
            }
            else
            {
                $send = LANG['users']['new']['msg']['login_email']['err'];
            }
            ...
            $response_array['title'] = LANG['users']['new']['msg']['login_email']['label'];
        ...
        else
        {
            ...
            $response_array['title'] = LANG['users']['edit']['msg']['err']['label'];
            $response_array['body'] = LANG['users']['edit']['msg']['err']['text'];
    ...
    public function updateUser($post_val)
    {
        ...
        if ($conn->query("UPDATE app_users SET ?u WHERE user_id = ?i", $query_data, $post_val['user_id']))
        {
            ...
            $response_array['type'] = 'success';
            $response_array['title'] = LANG['user']['acc_update']['msg']['update']['label'];
            $response_array['body'] = 'User <b>' . $post_val['user_email'] . '</b> ' . LANG['user']['acc_update']['msg']['update']['text'];
        ...
        else
        {
            ...
            $err_lvl = 2;
            $response_array['type'] = 'error';
            $response_array['title'] = LANG['user']['acc_update']['msg']['err']['label'];
            $response_array['body'] = LANG['user']['acc_update']['msg']['err']['text'];
    ...
    public function deleteUser($post_val)
    {
        ...
        if ($conn->query("DELETE FROM app_users WHERE user_id = ?i", $post_val['user_id']))
        {
            ...
            $response_array['type'] = 'success';
            $response_array['title'] = LANG['users']['edit']['msg']['del']['label'];
            $response_array['body'] = 'User <b>' . $user_email . '</b> ' . LANG['users']['edit']['msg']['del']['text'];

        }
        else
        {
            $msg = "User " . $user_email . " NIET verwijderd";
            $err_lvl = 2;
            $response_array['type'] = 'error';
            $response_array['title'] = LANG['users']['edit']['msg']['err']['label'];
            $response_array['body'] = LANG['users']['edit']['msg']['err']['text'];
        ...
        $columns = array(
            ...
            array(
                ...
                'formatter' => function ($d, $row)
                {
                    ...
                    if ($conn->getOne("select 1 from app_users WHERE user_last_access > ?s AND `user_name` = ?s AND `user_status` NOT IN ('Blocked')", $t, $d))
                    {
            ...
            array(
                ...
                'formatter' => function ($d, $row)
                {
                    if ($d == 'Active')
                    {
                        $status = '<span class="label label-success">' . LANG['users']['status']['active'] . '</span>';
                    }
                    else
                    {
                        $status = '<span class="label label-danger">' . LANG['users']['status']['inactive'] . '</span>';
                    }
                    return $status;
                }
            ) ,
