...
* - getSignalLoad()
* - getPendingEvents()
* - getPendingEventsGouped()
* - getLocationSignalCount()
* - getPortMonitor()
* - getPortMonitorAoip()
* - getPortMonitorBlock()
* - setPortState()
* - getAlarmDescriptionCount()
* - getLocationService()
* - getSignalLoadWeeklyAvg()
* - setPriority()
* - trendLineAnalyse()
* Classes list:
* - Tools
...
class Tools
{
    ...
    public function getSignalLoadEvents($primair_conn = array())
    {
        ...
        if ($connected)
        {
            ...
            $period = new DatePeriod(new DateTime($past_24) , new DateInterval('PT1H') , new DateTime($now_24));
    ...
    public function getPendingEvents($event_type = '')
    {
        ...
        if ($event_type == 'TASK')
        {
            $where = $conn_scs->parse("WHERE Event_Type IN ('TASK') OR Alarm_Priority IN (91)");
            $order = $conn_scs->parse("ORDER BY `Alarm_Priority` ASC, `Alarm_Priority_Original` ASC, `DateTime` ASC");
        }
        else
        {
            $where = $conn_scs->parse("WHERE Event_Type NOT IN ('TRTS', 'TASK') AND Alarm_Priority NOT IN ('91')");
            $order = $conn_scs->parse("ORDER BY `Alarm_Priority` ASC, `DateTime` ASC");
        }
        ...
        $query = "SELECT
				...
				`Event_Reaction_Time`
				FROM `scs`.`scs_pending_events` ?p ?p";

        $res = $conn_scs->query($query, $where, $order);
        ...
        while ($row = $conn_scs->fetch($res))
        {
			...
			// Check if pending event are grouped. If so, don't show on pending events dashboard
    ...
    public function getPendingEventsGouped()
    {
        if ($this->enableAlarmThreshold == true)
        {
            ...
            {
                $event_text = '';
                $name = $this->db_conn->getOne("SELECT SCS_Dealer_Name FROM `scs`.`scs_dealer_status` WHERE SCS_Dealer_Code =?s", $group_name);
    ...
    public function getPortMonitor()
    {
        ...
        */
        ...
        foreach (PORT_ARR as $port => $portval)
        {
            ...
            // If AOIP is defined use other function
            if ($port == 'AOIP')
            {
                $blocks_asb .= $this->getPortMonitorAoip($portval['name'], $portval['fep_port'], $pri_conn, $backup_conn, $signal_conn);
            }
            else
            {
    ...
    {
        // Check if connection is available else show SQL not available error
        $opt_prim = array_merge($this->defaults, $primair_conn);
        ...
        if ($connected)
        {
            ...
            {
                // Check if fep port nr is defined in .ini file and parse sub query
                if ($fep_port != null)
                {
                    $wherepart = $conn_scs->parse("WHERE `SCS_Stat_Port_Fep` = ?i", $fep_port);
                    $errpart = $conn_scs->parse("AND `SCS_Stat_Port_Fep` = ?i", $fep_port);
                }
                // Main query to get port state and append to array
                $res = $conn_scs->query($query . " ?p ORDER BY `SCS_Stat_Port` ASC", $wherepart);
                while ($row = $conn_scs->fetch($res))
                {
                    $port_stat[] = $this->setPortState($row['SCS_Stat_Port_State']);
                }
                // Query to get error msgs if an server port is down
                $res_err = $conn_scs->query($query . " WHERE `SCS_Stat_Port_State` = 1 ?p ORDER BY `SCS_Stat_Port` ASC", $errpart);
            ...
            // Check backup connection of FEP
            $port_stat_b = array();
            if ($backup_conn != null)
            {
        ...
        // If err array contains messages show error msg block
        if (count($port_err) != 0)
        {
        ...
        while ($row_count = $conn_scs->fetch($count))
        {
            if ($row_count['Account_Group'] != '')
            {
                ...
                foreach ($event_type_arr as $a)
                {
                    ...
                    $bb[$a] = array(
                        'e_type_name' => $conn_scs->getOne("SELECT SCS_Alarm_Type_Short_Description FROM `scs`.`scs_alarm_type` WHERE SCS_Alarm_Type_Code = ?s", $a) ,
