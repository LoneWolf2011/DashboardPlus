    ...
    public function processLogin($conn, $post_val)
    {
        ...
        if (!empty($post_val['login']) && hash_equals($post_val['csrf'], $_SESSION['db_token']))
        {
            ...
            if ($login_ok)
            {
                ...
                $this
                    ->db_conn
                    ->query("DELETE FROM app_users_login_attempts WHERE user_id =?i", $id);
            ...
            else
            {
                ...
                $this
                    ->db_conn
                    ->query("INSERT INTO app_users_login_attempts SET ?u", $query_arr);
        ...
        if (!empty($_POST['recover']) && hash_equals($_POST['csrf'], $_SESSION['db_token']))
        {
            ...
            if ($row)
            {
                ...
                if ($_SERVER["REQUEST_TIME"] - $row['user_date_time'] > $delta)
                {
                    ...
                    $this
                        ->db_conn
                        ->query("DELETE FROM app_users_tokens WHERE user_email = ?s ", $cleaned_email);
            ...
            if ($token_auth)
            {
                ...
                try
                {
                    ...
                    $this
                        ->db_conn
                        ->query("UPDATE app_users SET user_password = ?s, user_new = ?i WHERE user_id = ?i", $hash, $new_user, $row['user_id']);
