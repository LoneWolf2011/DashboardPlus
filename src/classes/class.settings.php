<?php
	...
	class Settings {
		...
		public function updateSiteZones($post_val){
			...
			if($conn->query("UPDATE sensor_sites_group SET ?u WHERE `Group` IN (?a)", $query_data, $post_val['add_zones'])) {
				
				// Log to file
				$msg = "Site settings geupdatet door: ".$this->auth_user;
				$err_lvl =0;
				
				$response_array['type'] 	= 'success';				
				$response_array['title'] 	= 'Success';				
				$response_array['body'] 	= 'Zones gewijzigd voor <b>site'.$post_val['select_site'].'</b>';	
				
			} else {
				$msg = "Site settings niet geupdatet ";
				$err_lvl=2;
